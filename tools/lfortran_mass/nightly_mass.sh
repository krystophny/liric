...
usage: nightly_mass.sh [options]
  ...
  --runtime-bc PATH         runtime bitcode path for liric (auto-generated by bench_compat_check if missing)
...
if [[ -z "$compat_jsonl" ]]; then
    ...
    if [[ -z "$runtime_bc" ]]; then
        runtime_bc="${bench_dir}/runtime/lfortran_intrinsics.bc"
    fi
    bench_args=(
        "$bench_compat_check"
        --timeout "$timeout_sec"
