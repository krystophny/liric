...
int test_wasm_ir_i64_unsigned_div_rem_lower_to_integer_ops(void) {
    ...
    for (lr_inst_t *inst = f->first_block->first; inst; inst = inst->next)
        if (inst->op == LR_OP_UDIV)
            found_udiv = true;
    TEST_ASSERT(found_udiv, "i64.div_u lowers to unsigned div op");
    ...
    f = m->first_func;
    bool found_urem = false;
    TEST_ASSERT(f != NULL && f->first_block, "has i64.rem_u body");
    for (lr_inst_t *inst = f->first_block->first; inst; inst = inst->next)
        if (inst->op == LR_OP_UREM)
            found_urem = true;
    TEST_ASSERT(found_urem, "i64.rem_u lowers to unsigned rem op");
